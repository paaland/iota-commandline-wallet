...
var walletName; // filename of the wallet programm itself
...
async function main() 
{
    ...
    if (process.argv.length < 3) {
        error_output("You need to specify wallet name as first argument. That wallet must be configured in file iota-wallet-config.js");
        return false;
    }

    walletName = process.argv[2];
    
    if (config[walletName] == undefined)
    {
        error_output("no config section named  "+walletName+"  in file iota-wallet-config.js found, please add one");
        return false;
    }    

    debug_output("Using wallet: " + walletName, 9);

    var db;
    try
    {
        db = new Datastore({ filename: config[walletName].databaseFile, autoload: true });
    }
    catch(e)
    {
    ...
    var seed = config[walletName].seed;
    if ( seed == "")
...
function showcommands(version)
{
    ...
    debug_output("Syntax: node iota-wallet.js <wallet name> <command> [parameter1] [parameter2] [parameter...]",0);
    debug_output("Command               Parametrs           Purpose",0);
    debug_output("SyncAll               AddressIndex        (re)Builds/creates the local Database, up to given address index (10 by default).",0);
    ...
    debug_output("node iota-wallet.js my-wallet Transfer DESTINATION9ADDRESS9SOME9MORE9TRYTES9OF9THE9DESTINATION9ADDRESS 10",0);
    debug_output("node iota-wallet.js my-wallet GetBundles DESTINATION9ADDRESS9SOME9MORE9TRYTES9OF9THE9DESTINATION9ADDRESS",0);
    debug_output("node iota-wallet.js my-wallet GetConfirmationState THE9BUNDLE9HASH9KJZTXIIHIHDXOFKJOIJIJOIJIMIM9NECWOI",0);
    debug_output("node iota-wallet.js my-wallet Replay DESTINATION9ADDRESS9SOME9MORE9TRYTES9OF9THE9DESTINATION9ADDRESS",0);
}


function debug_output(message, loglevel)
{
    if (!config[walletName] || (loglevel <= config[walletName].debugLevel))    
    {
        console.log(message);
    }
}
        ...
        else
        {
            ...
            while (total < value && i < rows.length)
            {
                ...
                if (rows[i].address != undefined && iota.valid.isAddress(rows[i].address) && rows[i].balance > 0)
                {
                    ...
                    { input.security = config[walletName].addressSecurityLevel; }
                    else
...
async function addNewAddress(db_handle,iota,seed,adr_index)
{
    ...
    var data = { index: adr_index, address: address, balance: 0, status: "new", securityLevel: config[walletName].addressSecurityLevel };
...
function getNewAddress(iotaHandle,seed,index)
{
    return new Promise(
        function(resolve, reject)
        {
            iotaHandle.api.getNewAddress(seed , {"index": index, "checksum": true, "total": 1, "security": config[walletName].addressSecurityLevel, "returnAll": false},
                function(error, success) {
                    if (error) {
                        reject(error);
                    } else {
